...
data['age_bucket'] = pd.cut(data.PatientAge, bins=[17,34,44,49,54,59,64,69,74,79,84,100])
data["group"] = pd.Categorical(data.age_bucket.astype(str) + data.Gender.astype(str)).codes
...
ecg_data = ECGDataset('condition', '../../data/ECGDataDenoised/',data)
...
for x,y,group in ecg_loader:
    equality_matrix = 1*(group[None,:]==group[:,None]) - torch.eye(batch_size,batch_size)
