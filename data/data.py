...
def get_data(args):
	...
	train_loader = DataLoader(ECGDataset(args, train_df), batch_size=args.batch_size, shuffle=True, num_workers=2)
	val_loader = DataLoader(ECGDataset(args, val_df), batch_size=args.batch_size, shuffle=True, num_workers=2)
	test_loader = DataLoader(ECGDataset(args, train_df), batch_size=args.batch_size, shuffle=False, num_workers=2)
...
def save_trainid(args):
	...
	val_ids = uids[int((frac_train)*len(uids)):int((frac_train+frac_val)*len(uids))]
